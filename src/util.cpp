...
const char * const BITCOIN_CONF_FILENAME = "stannum.conf";
const char * const BITCOIN_PID_FILENAME = "stannumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stannum" is a composite category enabling all Stannum-related debug output
            if(ptrCategory->count(string("stannum"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StannumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StannumCore
    // Mac: ~/Library/Application Support/StannumCore
    // Unix: ~/.stannumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StannumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StannumCore";
#else
    // Unix
    return pathRet / ".stannumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stannum.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stannum.conf
